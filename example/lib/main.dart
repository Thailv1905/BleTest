...
import 'package:flutter_blue_example/ble_model.dart';
...
class _DeviceState extends State<DeviceScreen>{
  ...
  void initState() {
    ...
    widget.device.state.listen((event) {
      switch (event.name) {
        case 'connected':
          var checkId = BleData(data: SendData(objectSend: "ev_id",value: "CM000"));
          widget.device.write(checkId.convert());
          // widget.device.services.listen((event) {
          //   print("Service: ${event.length}");
          // });
          break;
      }
    });
